...
import { decrementQuantity, incrementQuantity, removeFromCart, selectCartTotal, selectTotalAfterDiscount, selectTotalDiscountPercentage, selectTotalSavings } from "@/reduxToolkit/slices/cartSlice";
                    ...
                    <div className="flex flex-wrap gap-4 items-center justify-between border-b pb-4" key={i}>
                      ...
                      <div className="flex items-center space-x-10">
                        <div className="flex items-center border rounded-lg">
                          <button className="px-2 py-1 bg-red-500 text-white" onClick={() => dispatch(decrementQuantity(item._id))}>
                            −
                          </button>
                          <span className="px-4">{item?.quantity}</span>
                          <button className="px-2 py-1 bg-green-500 text-white" onClick={() => dispatch(incrementQuantity(item._id))}>
                            +
                          </button>
                        </div>
                        <div>
                          ...
                          <p className="text-green-600 text-sm">You Save {item?.percentage}%</p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-10">
                        <p className="text-lg font-bold">₹{item?.price}</p>
                        <button className="text-red-500" onClick={() => dispatch(removeFromCart(item?._id))}>🗑️</button>
                      </div>
                    </div>
                  ))}
                </div>
