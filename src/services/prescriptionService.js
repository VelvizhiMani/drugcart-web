...
const GetPrescriptionService = (page = 1, limit, search = "") => async (dispatch) => {
    try {
        dispatch(IsLoading(true))
        const getData = await axios.get(`/api/prescription?page=${page}&limit=${limit}&search=${search}`, { headers: await Authorization() })
